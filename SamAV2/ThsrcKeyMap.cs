public class ThsrcKeyMap
{
    public const byte THSRC_MK_BASE = 105; //THSRC Mifare Key Base is Key Entry 105.

    /// <summary>
    /// 由 Block number 計算在高鐵卡片在 SAM AV2 對應的 KeyNo
    /// </summary>
    ...
    /// <summary>
    /// 由 Block number 計算在高鐵卡片在 SAM AV2 對應的 KeyVer
    ...
    /// <summary>
    /// 由 Sector Number 計算在高鐵卡片在 SAM AV2 對應的 KeyNo
    /// </summary>
    ...
    public static byte SectorToKeyNo(byte SecNo, byte KeyType)
    {
        ...
        return (byte)(KeyOffset + THSRC_MK_BASE);
    }

    /// <summary>
    /// 由 Sector number 計算在高鐵卡片在 SAM AV2 對應的 KeyVer
    /// 註: 本函式提供的 KeyVer 的算法, 是針對高鐵 SAM AV2的設定特性, 而提供的簡化算法.
    /// 這不是正確的換算方式, 只能算特例, 正確的做法應應該是到 Key Entry 內取得對應的 KeyVer
    /// </summary>
    /// <param name="SecNo"> Mifare card sector number, 0 ~ 15. </param>
